...
class report_model extends CI_Model {
    ...
    function get_report_user_count($rId)
    {
        $sql = "select * from reports where patient_id='$rId'";
        $result=$this->db->query( $sql );
    ...
    function report_list($nPerPage = 255, $nOffset = 0)
    {
		$sql="SELECT reports.report_id,reports.report_name,patients.patient_fullname,patients.patient_lastname, patients.email, patients.date_added FROM reports 
                   inner join patients where reports.patient_id = patients.patient_id limit $nPerPage offset $nOffset";
        $result=$this->db->query( $sql );
        return $result->result_array();
    }
    ...
    {
        $sql = "SELECT reports.report_id, reports.report_name, reports.patient_id, patient_report.id, patient_report.test_name, patient_report.test_result FROM reports 
                       INNER JOIN patient_report WHERE reports.report_id=patient_report.report_id AND reports.report_id ='$rId' ";
        $result=$this->db->query( $sql );
        return $result->result_array();
    }
	...
	function get_patient_report($pId)
    {
        ...
        INNER JOIN patient_report ON reports.report_id = patient_report.report_id  WHERE reports.patient_id ='$pId'";
